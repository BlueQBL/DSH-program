...
**五套主题**（校样 / 石墨 / 夜读 / 蓝图 / 苔原，换的是整套 token；色板从 `getComputedStyle` 读回来，
CSS 是唯一真相）、**四种 Markdown 风格**（通用 / 中文写作 / 全扩展 / 严格 CommonMark，
13 个扩展开关——含中英之间自动留白、单换行即换行、`==高亮==`、上下标、定义列表）。
...
质量上：`npm test` 跑 **解析层 258 + 公式层 139 + 打包层 31 + 图片层 95 + 数据层 124 = 647 项断言**
...
流程探针像用户那样打字、搜索、切笔记、删光再撤销、点勾选框、**上传图片再撤销删除**、换主题、换风格、
**拆掉外层围栏**、**导出 ZIP 并把产物读回来看目录结构**，共 **114 项断言**（这个探针跑真实时钟：
