...
导出 `.md` 与 JSON 备份 / 导入、拖中缝调分栏、窄屏三栏切换、`Ctrl+P` 只印印张。

Markdown 解析器与语法高亮都是手写的（零依赖，约 700 行）：解析器给每个块级元素打上 `data-line` / `data-end`
行号区间（光标双向联动的前提），高亮覆盖 10 种语言 + 未知语言通用退回，token 配色服从整套配色而不是反过来；
安全上原文 HTML 一律转义、链接走协议白名单，避免笔记内容变成 XSS。

质量上：`npm test` 跑 **127 项解析层 + 112 项数据层**断言；另有两个开发探针——
版面探针把应用塞进 iframe 量三栏几何 / 字体 / 溢出 / 记号对齐，流程探针像用户那样打字、搜索、切笔记、
删光再撤销、点勾选框，**45 项断言**；还有一个自写 PNG 解码脚本做区域统计（底色、有没有字、红笔在哪）。
详见 [markdown-notes/README.md](markdown-notes/README.md)。
